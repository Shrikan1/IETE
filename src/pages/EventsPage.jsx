...
const EVENTS = [
  {
    id: 1,
    title: 'CircuitCraft Championship',
    dept: 'Electronics Wing',
    date: 'Mar 22, 2026',
    location: 'Lab Block 3, IETE Campus',
    type: 'Competition',
    status: 'upcoming',
    color: '#08CB00',
    bg: '#0d3b30',
    img: 'https://images.unsplash.com/photo-1518770660439-4636190af475?w=800&q=80',
    desc: 'A hands-on circuit design competition where teams build, test, and compete using embedded systems and custom PCBs. Push your hardware skills to the limit.',
    speakers: ['Prof. R. Sharma', 'Eng. Kavitha M.'],
    seats: 120,
    registered: 94,
  },
  {
    id: 2,
    title: 'HackSphere 3.0',
    dept: 'Software Wing',
    date: 'Apr 05, 2026',
    ...
    desc: '36-hour non-stop hackathon. Build innovative solutions across fintech, health-tech, and smart city domains. ₹1,00,000 prize pool awaits.',
    speakers: ['Arjun Nair (CTO, StartX)', 'Priya Rao (Google DevExpert)'],
    seats: 200,
    registered: 178,
  },
  {
    ...
    desc: 'A 3-day immersive workshop covering neural networks, computer vision, and large language models. Hands-on sessions with real datasets and projects.',
    speakers: ['Dr. Ananya Singh', 'Rohan Verma (MLOps Lead)'],
    seats: 80,
    registered: 80,
  },
  {
    ...
    date: 'May 10, 2026',
    location: 'Maker Space, Lab Block 1',
    type: 'Workshop',
    status: 'upcoming',
    color: '#38bdf8',
    bg: '#03111e',
    img: 'https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=800&q=80',
    desc: 'Learn to build IoT prototypes for smart city applications using Raspberry Pi, MQTT, and cloud dashboards. Take home a working prototype.',
    speakers: ['Vikram C.', 'Neha Joshi (Bosch IoT)'],
    seats: 60,
    registered: 41,
  },
];
...
const PAST_EVENTS = [
  ...
  },
  {
    ...
    highlight: '12 Companies · 500+ Students',
  },
  {
    id: 9,
    ...
    highlight: '30 Robots · National Level',
  },
  {
    id: 10,
    title: 'Paper Presentation Summit',
    date: 'Aug 20, 2025',
    img: 'https://images.unsplash.com/photo-1475721027785-f74eccf877e2?w=800&q=80',
    highlight: '80 Papers · 6 Tracks',
  },
];

const STATS = [
  { val: 500, suffix: '+', label: 'Active Members' },
  { val: 30, suffix: '+', label: 'Events Per Year' },
  { val: 10, suffix: '+', label: 'Workshops Held' },
...
function EventCard({ ev, index, visible }) {
  ...
  const progress = Math.round((ev.registered / ev.seats) * 100);
  ...
  return (
    <>
      ...
      >
        {/* Image */}
        <div style={{ position: 'relative', height: 200, overflow: 'hidden', borderTopLeftRadius: 20, borderTopRightRadius: 20 }}>
          <img src={ev.img} alt={ev.title} style={{ width: '100%', height: '100%', objectFit: 'cover', transition: 'transform 0.5s', display: 'block' }} />
          <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(7,7,8,0.7) 0%, transparent 60%)' }} />
          <span style={{ position: 'absolute', top: 14, right: 14, fontSize: 10, fontWeight: 800, letterSpacing: '0.1em', textTransform: 'uppercase', padding: '5px 12px', borderRadius: 100, background: ev.color, color: '#fff', boxShadow: '0 2px 8px rgba(0,0,0,0.12)' }}>{ev.type}</span>
        </div>

        {/* Body */}
        <div style={{ padding: '22px 22px 26px', display: 'flex', flexDirection: 'column', gap: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            <span style={{ color: ev.color, fontWeight: 700, fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.15em' }}>{ev.dept}</span>
            <span style={{ fontSize: 12, color: '#fff', background: ev.color, borderRadius: 8, padding: '2px 8px', fontWeight: 600 }}>{ev.type}</span>
          </div>
          ...
          <p style={{ fontSize: 13.5, color: 'rgba(255,255,255,0.7)', lineHeight: 1.65, marginBottom: 12 }}>{ev.desc}</p>

          {/* Meta row */}
          <div style={{ display: 'flex', gap: 16, fontSize: 13, color: 'rgba(255,255,255,0.5)', marginBottom: 10 }}>
            <span>📅 {ev.date}</span>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <div style={{ background: 'rgba(255,255,255,0.12)', borderRadius: 4, height: 6, overflow: 'hidden' }}>
              <div style={{ height: '100%', borderRadius: 4, width: visible ? `${progress}%` : '0%', background: ev.color, transition: 'width 1s cubic-bezier(0.22,1,0.36,1) 0.3s' }} />
            </div>
          </div>
          ...
          <button
            style={{
              background: isFull ? 'rgba(255,255,255,0.08)' : ev.color,
              color: isFull ? 'rgba(255,255,255,0.4)' : '#fff',
              ...
              boxShadow: isFull ? 'none' : `0 2px 12px ${ev.color}55`,
      ...
      {open && (
        <div className="ep-modal-backdrop" onClick={() => setOpen(false)}>
          <div className="ep-modal" onClick={(e) => e.stopPropagation()}>
            <button className="ep-modal-close" onClick={() => setOpen(false)}>✕</button>
            <div className="ep-modal-img-wrap">
              <img src={ev.img} alt={ev.title} />
              <div className="ep-modal-img-grad" />
              <span className="ep-ev-type-badge" style={{ background: ev.color, color: '#fff', position: 'absolute', top: 20, left: 20 }}>
                {ev.type}
              </span>
            </div>
            <div className="ep-modal-body">
              <p className="ep-ev-dept" style={{ color: ev.color, marginBottom: 8 }}>{ev.dept}</p>
              <h2 className="ep-modal-title">{ev.title}</h2>
              <p className="ep-modal-desc">{ev.desc}</p>
              <div className="ep-modal-meta">
                <div className="ep-modal-meta-item"><span className="ep-modal-meta-icon">📅</span><div><b>Date</b><span>{ev.date}</span></div></div>
                <div className="ep-modal-meta-item"><span className="ep-modal-meta-icon">📍</span><div><b>Location</b><span>{ev.location}</span></div></div>
                <div className="ep-modal-meta-item"><span className="ep-modal-meta-icon">🎤</span><div><b>Speakers</b><span>{ev.speakers.join(', ')}</span></div></div>
                <div className="ep-modal-meta-item"><span className="ep-modal-meta-icon">🎟️</span><div><b>Seats</b><span>{ev.registered}/{ev.seats} ({progress}% filled)</span></div></div>
              </div>
              <div className="ep-reg-bar-bg" style={{ marginTop: 16 }}>
                <div className="ep-reg-bar-fill" style={{ width: `${progress}%`, background: ev.color }} />
              </div>
              <button
                className="ep-ev-btn"
                style={{
...
export default function EventsPage() {
  ...
  const upcomingVisible = useInView(upcomingRef, 0.1);
  const statsVisible = useInView(statsRef, 0.2);
  const pastVisible = useInView(pastRef, 0.1);
  const ctaVisible = useInView(ctaRef, 0.2);

  const [filter, setFilter] = useState('All');
  const filters = ['All', 'Competition', 'Hackathon', 'Workshop'];
  ...
  return (
    <div className="ep-page">
      ...
      <section className="ep-hero" ref={heroRef}>
        ...
        <div className="ep-hero-bg-ring" />
        <div
          className={`ep-hero-inner${heroVisible ? ' ep-visible' : ''}`}
          style={{ opacity: 0, transform: 'translateY(40px)' }}
        >
          <div className="ep-hero-eyebrow">IETE Student Forum</div>
          <h1 className="ep-hero-title">
            Where Ideas <br />Become <em>Events</em>
          </h1>
          <p className="ep-hero-sub">
            Competitions, workshops, hackathons, and more — crafted by students, for students.
            Find your next breakthrough moment here.
          </p>
          <div className="ep-hero-actions">
            <button className="ep-btn-primary">Explore Events ↓</button>
            <button className="ep-btn-outline">Host an Event →</button>
          </div>
      ...
      {/* ── Upcoming Events ── */}
      <div className="ep-section" ref={upcomingRef}>
        <div className="ep-section-header">
          <p className="ep-section-label">What's Coming Up</p>
          <h2 className="ep-section-title">Upcoming Events</h2>
          <p className="ep-section-sub">
            Register early — seats fill fast. Click any card to see the full event details.
          </p>
        </div>
        <div className="ep-filters">
          {filters.map(f => (
            <button
              key={f}
              className={`ep-filter-btn${filter === f ? ' ep-filter-active' : ''}`}
              onClick={() => setFilter(f)}
            >
              {f}
            </button>
        ...
        <div className="ep-ev-grid">
          {filtered.map((ev, i) => (
            <EventCard key={ev.id} ev={ev} index={i} visible={upcomingVisible} />
          ))}
        </div>
      </div>

      {/* ── Stats strip ── */}
      <div className="ep-stats-strip" ref={statsRef}>
      ...
      {/* ── Past Events ── */}
      <div className="ep-section" ref={pastRef}>
        <div className="ep-section-header">
          <p className="ep-section-label">Look Back</p>
          <h2 className="ep-section-title">Past Events Gallery</h2>
          <p className="ep-section-sub">
            Memories we made together. Hover each photo to relive the moment.
          </p>
        </div>
        <div className="ep-past-grid">
          {PAST_EVENTS.map((ev) => (
            <div key={ev.id} className={`ep-past-card${pastVisible ? ' ep-in' : ''}`}>
              <img src={ev.img} alt={ev.title} loading="lazy" />
              <div className="ep-past-overlay">
                <h4>{ev.title}</h4>
                <p>{ev.highlight}</p>
              </div>
              <div className="ep-past-bar">
                <span>{ev.date}</span>
              </div>
            </div>
          ))}
      ...
      {/* Host Your Own Event section removed as requested */}
    </div>
  );
}
